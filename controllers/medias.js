...
router.get('/:itemId', isSignedIn, isOwner, async (req, res) => {
  try {
    ...
    res.render('medias/show', {
      media,
      user,
      noteError: req.query.noteError || null,
    });
  } catch (err) {
    console.error(err);
...
router.post('/:itemId/notes', isSignedIn, isOwner, async (req, res) => {
  try {
    ...
    if (!text)
      return res.redirect(
        `/users/${user._id}/medias/${media._id}?noteError=empty`
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.error(err);
      res.redirect(
        `/users/${req.session.user._id}/medias/${req.params.itemId}`
      );
    }
  }
);
